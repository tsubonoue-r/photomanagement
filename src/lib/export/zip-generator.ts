  ...
  return new Promise((resolve, reject) => {
    ...
    archive.on('end', () => {
      ...
      resolve({
        buffer,
        filename,
        fileCount: album.photos.length + 3, // photos + README + album.json + photos.json
      });
    });
    ...
    // Add photo placeholder files (in real implementation, would download actual photos)
    album.photos.forEach((photo, index) => {
      const photoInfo = generatePhotoInfo(photo, index, options);
      const filename = `photos/${String(index + 1).padStart(3, '0')}_${photo.title.replace(/[^a-zA-Z0-9]/g, '_')}.txt`;
      archive.append(photoInfo, { name: filename });
    });
